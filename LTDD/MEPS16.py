...
# Load dataset
dataset_orig = pd.read_csv('datasets\\h192.csv')

# Rename columns to match expected format
dataset_orig.rename(columns={"UTILIZATION": "Probability", "RACE": "race"}, inplace=True)
...
for k in range(100):
    ...
    for i in range(len(column_u)):
        X_test[column_u[i]] = X_test[column_u[i]] - Linear_regression(X_test['race'], slope_store[i], intercept_store[i])
